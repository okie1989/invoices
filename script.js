...
printBtn.onclick = function() {
    ...
    win.document.write(renderInvoiceView(data));
...
searchBar.addEventListener('input', async function() {
    ...
    invoices.forEach(inv => {
        const fields = [
            inv.invoiceNumber, inv.customerName, inv.orderName, inv.invoiceDate, inv.dateline,
            ...(inv.items || []).map(i => i.name + ' ' + i.size)
        ].join(' ').toLowerCase();
        if (fields.includes(q)) {
            results.push({
                type: 'invoice',
                label: `<b>Invoice:</b> ${highlightKeyword(inv.invoiceNumber, q)} - ${highlightKeyword(inv.customerName, q)} - ${highlightKeyword(inv.orderName, q)}`,
                ref: inv.invoiceNumber
            });
        }
    });
    ...
    expenses.forEach(exp => {
        const fields = [
            exp.date, exp.type, exp.supplier,
            ...(exp.items || []).map(i => i.name)
        ].join(' ').toLowerCase();
        if (fields.includes(q)) {
            results.push({
                type: 'expense',
                label: `<b>Pengeluaran:</b> ${highlightKeyword(exp.date, q)} - ${highlightKeyword(exp.type, q)} - ${highlightKeyword(exp.supplier, q)}`,
