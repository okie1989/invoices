...
// Satuan list
const satuanList = ['Pcs', 'Kg', 'Roll', 'Cm', 'Meter', 'Lembar', 'Set', 'Box', 'Pack', 'Liter', 'Unit', 'Dus', 'Kodi', 'Lainnya'];

// Item row template
function createItemRow(item = {}) {
    const tr = document.createElement('tr');
    tr.innerHTML = `
        <td><input type="text" class="item-name" value="${item.name || ''}" style="width:220px;min-width:160px;max-width:320px;"></td>
        <td><input type="text" class="item-size" value="${item.size || ''}" style="width:70px;min-width:60px;max-width:90px;text-align:center;"></td>
        <td>
            <input type="number" class="item-qty" value="${item.qty || 1}" min="0" step="any" style="width:70px;min-width:60px;max-width:90px;text-align:center;">
        </td>
        <td>
            <select class="item-unit" style="width:70px;min-width:60px;max-width:90px;text-align:center;">
                ${satuanList.map(s => `<option value="${s}"${item.unit === s ? ' selected' : ''}>${s}</option>`).join('')}
            </select>
        </td>
        <td><input type="number" class="item-price" value="${item.price || 0}" min="0" step="any"></td>
...
function normalizeInvoice(inv) {
    return {
        ...
        paidAmount: normalizeNumber(inv.paidAmount || inv['paid-amount'] || inv.amountPaid),
...
const firebaseConfig = {
  ...
  databaseURL: "https://invoicenew-37e56-default-rtdb.asia-southeast1.firebasedatabase.app",
...
async function setAutoInvoiceNumber() {
    ...
    if (!invoiceNumberInput.value || invoiceNumberInput.value.startsWith('INV-')) {
...
document.getElementById('customer-name').addEventListener('input', setAutoInvoiceNumber);
...
if (paidFullBtn) {
    paidFullBtn.onclick = function() {
        ...
        const total = Number(totalPrice.textContent.toString().replace(/[^\d,]/g, '').replace(',', '.')) || 0;
        const disc = Number(discount.value) || 0;
...
function collectInvoiceData() {
    ...
    const items = Array.from(orderItems.querySelectorAll('tr')).map(tr => ({
    ...
    return {
        ...
        unpaidAmount: unpaid,
        changeAmount: change,
        businessName: document.querySelector('.business-name') ? document.querySelector('.business-name').innerHTML : '',
        logo: document.querySelector('.logo') ? document.querySelector('.logo').src : '',
...
function fillInvoiceData(data) {
    document.getElementById('invoice-date').value = data.invoiceDate || data.date || '';
    ...
    document.getElementById('paid-amount').value = data.paidAmount || data.amountPaid || 0;
...
viewBtn.onclick = function() {
    const data = collectInvoiceData();
    // Tampilkan modal tanpa label dan tanpa tombol hapus nota tersimpan
    // Tambahkan tombol download di atas invoice (hapus tombol share WhatsApp)
    const controlsHtml = `
    ...
    document.getElementById('download-invoice-jpg').onclick = downloadInvoiceAsJPG;
};
...
printBtn.onclick = function() {
    ...
    function renderStruk(data) {
        ...
        const address = 'Jl.Riau Barat No.21 Kec. Sananwetan Kota Blitar<br>Telp. 082257423324';
        ...
        groupedRows.forEach(row => {
            ...
            htmlRows += `
                <tr>
                    <td style="text-align:left;word-break:break-all;">${row.name !== prevName ? row.name : ''}</td>
                    <td style="text-align:right;">${row.qty}</td>
                    <td style="text-align:right;">${formatNumber(row.price)}</td>
                    <td style="text-align:right;">${formatNumber(row.subtotal)}</td>
                </tr>
            `;
            prevName = row.name;
        });
        ...
        <div style="width:50mm;max-width:100vw;font-family:monospace,sans-serif;font-size:10px;line-height:1.3;background:#fff;color:#111;padding:0;margin:0;">
            ...
            ${showAddress ? `<div style="text-align:center;margin-bottom:2px;">${address}</div>` : ''}
...
function showExpenseForm(expenseData = null, expenseKey = null) {
    const html = `
        ...
        <form id="expense-form" ${expenseKey ? `data-expense-key="${expenseKey}"` : ''}>
            <div>
                <label>Tanggal:</label>
                <input type="date" id="expense-date" required value="${new Date().toISOString().slice(0,10)}">
            </div>
            <div>
    ...
    function createExpenseItemRow(item = {}) {
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><input type="text" class="expense-item-name" value="${item.name || ''}" required></td>
            <td><input type="number" class="expense-item-qty" value="${item.qty || 1}" min="0" step="any" required style="width:60px;"></td>
            <td>
                <select class="expense-item-unit" style="width:70px;">
                    ${satuanList.map(s => `<option value="${s}"${item.unit === s ? ' selected' : ''}>${s}</option>`).join('')}
                </select>
            </td>
            <td><input type="number" class="expense-item-price" value="${item.price || 0}" min="0" step="any" required style="width:90px;"></td>
    ...
    document.getElementById('add-expense-item').onclick = () => addExpenseItemRow();
    ...
    function updateExpenseItemsTotal() {
        ...
        expenseItemsBody.querySelectorAll('tr').forEach(tr => {
            ...
            const subtotal = qty * price;
            tr.querySelector('.expense-item-total').textContent = formatNumber(subtotal);
            total += subtotal;
        });
    ...
    if (expenseData) {
        ...
        document.getElementById('expense-supplier').value = expenseData.supplier || '';
        // Jenis
        let found = false;
        ...
        expenseItemsBody.querySelectorAll('tr').forEach(tr2 => {
            ...
            const subtotal = qty * price;
            tr2.querySelector('.expense-item-total').textContent = formatNumber(subtotal);
            total += subtotal;
        });
        document.getElementById('expense-amount').value = total;
    }
...
async function saveExpense(e) {
    ...
    const typeManual = document.getElementById('expense-type-manual').value.trim();
    if (type === 'Lain-lain' && typeManual) type = typeManual;
    const supplier = document.getElementById('expense-supplier').value.trim();

    // Ambil array barang
    const items = Array.from(document.querySelectorAll('#expense-items-body tr')).map(tr => ({
    ...
    if (!type || !supplier || !items.length) return alert('Lengkapi semua data dan minimal 1 barang!');
    ...
    await saveExpenseToFirebase({ date, type, supplier, items, amount }, expenseKey);
...
async function renderExpenseList() {
    ...
    if (!expenses.length) {
        document.getElementById('expense-list-section').innerHTML = '<i>Belum ada pengeluaran tercatat.</i>';
        return;
    }
        ...
        <table style="width:100%;font-size:1em;">
                                ...
                                <tbody>
                                    ${(e.items||[]).map(item => `
                                        <tr>
                                            ...
                                            <td style="text-align:right;">${formatNumber(item.qty * item.price)}</td>
                                        </tr>
                                    `).join('')}
                                </tbody>
                            </table>
                        </td>
                        <td style="text-align:right;">${formatNumber(e.amount)}</td>
                        <td>${e.type || '-'}</td>
            ...
            <tfoot>
                <tr>
                    ...
                    <td style="font-weight:bold;text-align:right;">${formatNumber(total)}</td>
                    <td colspan="2"></td>
                </tr>
            </tfoot>
        </table>
...
bookkeepingBtn.onclick = async function() {
    ...
    if (val) {
        ...
        expenses = Object.values(val).map((e, i) => ({ ...e, _key: expenseKeys[i] }));
    }
    ...
    if (!uniqueMonths.includes(defaultMonth) && uniqueMonths.length) defaultMonth = uniqueMonths[0];
    ...
    async function renderBookkeepingTable(selectedMonth, highlightRef = null, highlightType = null) {
        // Filter data
        let filteredInvoices = selectedMonth ? invoices.filter(inv => (inv.invoiceDate||'').slice(0,7) === selectedMonth) : invoices;
        let filteredExpenses = selectedMonth ? expenses.filter(exp => (exp.date||'').slice(0,7) === selectedMonth) : expenses;
        ...
        transactions = transactions.filter(t => t.date).sort((a, b) => (a.date > b.date ? 1 : (a.date < b.date ? -1 : 0)));

        // Hitung saldo berjalan
        let saldo = 0;
        let rows = transactions.map((t, idx) => {
            ...
            let highlightClass = '';
            if (highlightRef && highlightType && t.ref == highlightRef && t.type == highlightType) {
                highlightClass = 'bk-row-highlight';
            }
            return `<tr class="bk-row ${highlightClass}" data-type="${t.type}" data-idx="${idx}" data-ref="${t.ref}" style="cursor:pointer;">
                <td>${t.date}</td>
                <td>${t.subjek}</td>
        ...
        let totalExpense = transactions.reduce((sum, t) => sum + (t.expense || 0), 0);
        let saldoAkhir = saldo;

        // --- Next/Prev Button Logic ---
        let currentIdx = uniqueMonths.indexOf(selectedMonth || "");
        let prevMonth = currentIdx < uniqueMonths.length - 1 ? uniqueMonths[currentIdx + 1] : null;
        let nextMonth = currentIdx > 0 ? uniqueMonths[currentIdx - 1] : null;

        let navHtml = `
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                <button id="bk-prev-month" ${!prevMonth ? 'disabled' : ''} style="padding:4px 12px;">&lt; Prev</button>
                <span style="font-weight:bold;">${selectedMonth || 'Semua Bulan'}</span>
                <button id="bk-next-month" ${!nextMonth ? 'disabled' : ''} style="padding:4px 12px;">Next &gt;</button>
            </div>
        `;
        ...
        let filterHtml = `
            <div style="margin-bottom:12px;">
                ...
                <select id="bookkeeping-month">
                    ...
                    ${uniqueMonths.map(m => `<option value="${m}"${m===selectedMonth?' selected':''}>${m}</option>`).join('')}
                </select>
            </div>
        `;
        ...
        document.querySelectorAll('.delete-bk-expense').forEach(btn => {
            btn.onclick = async function() {
                if (confirm('Hapus pengeluaran ini?')) {
                    ...
                    if (val) {
                        expenseKeys = Object.keys(val);
                        newExpenses = Object.values(val).map((e, i) => ({ ...e, _key: expenseKeys[i] }));
                    }
        ...
        document.querySelectorAll('.bk-row').forEach(tr => {
            tr.addEventListener('click', async function(e) {
                ...
                let currentMonth = document.getElementById('bookkeeping-month') ? document.getElementById('bookkeeping-month').value : selectedMonth;
                lastEditedRef = t.ref;
                ...
                if (t.type === 'INVOICE') {
                    ...
                    if (inv) {
                        ...
                        saveBtn.onclick = async function() {
                            ...
                            setTimeout(() => {
                                renderBookkeepingTable(currentMonth, data.invoiceNumber, 'INVOICE');
                                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        if (form) {
                            ...
                            form.onsubmit = async function(e2) {
                                ...
                                if (type === 'Lain-lain' && typeManual) type = typeManual;
                                const supplier = document.getElementById('expense-supplier').value.trim();
                                const items = Array.from(document.querySelectorAll('#expense-items-body tr')).map(tr2 => ({
                                ...
                                if (!date || !type || !supplier || !items.length) return alert('Lengkapi semua data dan minimal 1 barang!');
                                const amount = items.reduce((sum, i) => sum + i.qty * i.price, 0);
                                const expenseKey = form.getAttribute('data-expense-key') || null;
                                await saveExpenseToFirebase({ date, type, supplier, items, amount }, expenseKey);
                                closeModalFunc();
                        ...
                        const modal = document.getElementById('modal');
                        if (modal) {
                            modal.querySelector('.modal-content').style.boxShadow = '0 0 0 4px #007bff';
                            setTimeout(() => { modal.querySelector('.modal-content').style.boxShadow = ''; }, 1200);
                        }
                    }, 200);
                }
            });
...
statisticBtn.onclick = async function() {
    ...
    let allYears = Array.from(new Set(allDates.map(d => (d||'').slice(0,4)))).filter(Boolean).sort().reverse();
    let currentYear = new Date().getFullYear().toString();
    if (!allYears.includes(currentYear) && allYears.length) currentYear = allYears[0];

    // Ambil semua bulan unik di tahun terpilih
    function getMonthsForYear(year) {
        return Array.from(new Set(
            allDates.filter(d => d && d.startsWith(year)).map(d => d.slice(0,7))
        )).sort().reverse();
    }
        ...
        <div style="margin-bottom:16px;display:flex;gap:18px;align-items:center;flex-wrap:wrap;">
            ...
            <label><b>Bulan:</b>
                <select id="stat-month" style="font-size:1em;margin-left:4px;">
                    <option value="">Semua</option>
                    ${getMonthsForYear(currentYear).map(m => `<option value="${m}"${m===currentMonth?' selected':''}>${m}</option>`).join('')}
                </select>
            </label>
            <label><b>Mode:</b>
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            if (window.Chart && window.Chart.instances) {
                Object.values(window.Chart.instances).forEach(c => { if (c && c.resize) c.resize(); });
            }
        }, 150);
    }, 50);
    ...
    async function renderChart() {
        ...
        let filteredInvoices = invoices.filter(inv => inv.invoiceDate && inv.invoiceDate.startsWith(year));
        let filteredExpenses = expenses.filter(exp => exp.date && exp.date.startsWith(year));

        if (month) {
            filteredInvoices = filteredInvoices.filter(inv => inv.invoiceDate.startsWith(month));
            filteredExpenses = filteredExpenses.filter(exp => exp.date.startsWith(month));
        }
        ...
        if (mode === 'minggu') {
            ...
            filteredInvoices.forEach(inv => {
                ...
                let key = `${inv.invoiceDate.slice(0,4)}-W${String(week).padStart(2,'0')}`;
        ...
        if (typeof window.Chart === 'undefined') {
            alert('Chart.js library is not loaded. Please include it in your project.');
            return;
        }
        chart = new window.Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    x: {
                        title: { display: true, text: mode === 'minggu' ? 'Minggu' : (mode === 'bulan' ? 'Bulan' : 'Tahun'), font: { size: 16 } },
                        ticks: { font: { size: 15 } }
                    },
                    y: {
                        beginAtZero: true,
        ...
        document.getElementById('stat-summary').innerHTML = `
            ...
            <b>Total Pengeluaran:</b> ${formatNumber(totalExpense)} &nbsp; | &nbsp;
    ...
    document.getElementById('stat-year').onchange = function() {
        ...
        monthSel.innerHTML = `<option value="">Semua</option>` + months.map(m => `<option value="${m}">${m}</option>`).join('');
...
utangPiutangBtn.onclick = async function() {
    ...
    let totalUnpaid = utangInvoices.reduce((sum, inv) => sum + (inv.unpaidAmount || 0), 0);

    let html = `
        <h2>Daftar Piutang (Nota Belum Lunas)</h2>
        <div style="margin-bottom:12px;">
            <b>Total Piutang:</b> <span style="color:#e74c3c;font-size:1.2em;">${formatNumber(totalUnpaid)}</span>
        </div>
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr>
            ...
            <tbody>
                ${utangInvoices.map(inv => `
                    <tr>
                        <td>${inv.invoiceDate || '-'}</td>
                        <td>${inv.invoiceNumber || '-'}</td>
                        <td>${inv.customerName || '-'}</td>
                        <td>${inv.orderName || '-'}</td>
                        <td style="text-align:right;">${formatNumber(inv.totalPrice)}</td>
                        <td style="text-align:right;">${formatNumber(inv.paidAmount)}</td>
                        <td style="text-align:right;color:#e74c3c;font-weight:bold;">${formatNumber(inv.unpaidAmount)}</td>
                        <td>${inv.dateline || '-'}</td>
                        <td>
                            <button class="pay-utang-btn" data-no="${inv.invoiceNumber}" style="background:#27ae60;color:#fff;padding:2px 10px;border-radius:3px;">Bayar</button>
                            <button class="view-invoice-btn" data-no="${inv.invoiceNumber}" style="background:#007bff;color:#fff;padding:2px 10px;border-radius:3px;margin-left:4px;">Lihat</button>
                        </td>
                    </tr>
                `).join('')}
            </tbody>
    ...
    document.querySelectorAll('.pay-utang-btn').forEach(btn => {
        btn.onclick = async function() {
            ...
            let html = `
                ...
                <div><b>Belum Terbayar:</b> <span style="color:#e74c3c;">${formatNumber(unpaid)}</span></div>
            ...
            document.getElementById('pay-utang-form').onsubmit = async function(e) {
                e.preventDefault();
                let payAmount = Number(document.getElementById('pay-utang-amount').value) || 0;
                ...
                let newUnpaid = Math.max(total - newPaid - (Number(inv.discount)||0), 0);
                let newChange = Math.max(newPaid + (Number(inv.discount)||0) - total, 0);
                let updated = {
                    ...inv,
                    paidAmount: newPaid,
...
searchBar.addEventListener('input', async function() {
    ...
    invoices.forEach(inv => {
        ...
        const itemNamesArr = (inv.items || []).map(i => ((i.name || '') + ' ' + (i.size || '')));
        ...
        let found =
            anyFieldAllWords(fields, qWords) ||
            matchedItems.length > 0;
        if (found) {
            // Only show matched items (if any), otherwise show invoice info
            let itemHtml = '';
            if (matchedItems.length > 0) {
                itemHtml = matchedItems.map(item =>
                    `<div style="margin-left:12px;font-size:0.97em;">
                        <span style="color:#555;">${highlightKeyword(item.name, q)}${item.size ? ' (' + highlightKeyword(item.size, q) + ')' : ''}</span>
                        <span style="color:#888;">Qty: ${item.qty} ${item.unit || ''}</span>
                    </div>`
                ).join('');
            }
            results.push({
                type: 'invoice',
                label: `<b>Invoice:</b> ${highlightKeyword(invoiceNumber, q)} - ${highlightKeyword(customerName, q)} - ${highlightKeyword(orderName, q)}${itemHtml}`,
                ref: invoiceNumber
            });
        }
    });
    ...
    expenses.forEach(exp => {
        ...
        if (found) {
            ...
            if (matchedItems.length > 0) {
                itemHtml = matchedItems.map(item =>
                    `<div style="margin-left:12px;font-size:0.97em;">
                        <span style="color:#555;">${highlightKeyword(item.name, q)}</span>
                        <span style="color:#888;">Qty: ${item.qty} ${item.unit || ''}</span>
                    </div>`
                ).join('');
            }
            results.push({
                type: 'expense',
                label: `<b>Pengeluaran:</b> ${highlightKeyword(date, q)} - ${highlightKeyword(type, q)} - ${highlightKeyword(supplier, q)}${itemHtml}`,
    ...
    if (!results.length) {
        searchResults.innerHTML = '<div style="padding:8px;">Tidak ditemukan.</div>';
...
searchResults.addEventListener('click', async function(e) {
    ...
    if (type === 'invoice') {
        ...
        setTimeout(async () => {
            ...
            const exp = expenses.find(e => e.date === date && e.supplier === supplier && e.type === typeVal);
            if (exp) {
...
function renderInvoiceView(data) {
    ...
    <div class="invoice-container invoice-view" style="max-width:900px;width:95vw;min-width:0;margin:24px auto;">
        ...
        <div class="header2 invoice-view-header2-flex">
            ...
            ${mockupHtml ? `<div class="invoice-view-header2-right">${mockupHtml}</div>` : ''}
        </div>
        <table style="margin-bottom:12px;">
            ...
            <tfoot>
                <tr>
                    ...
                    <td style="font-weight:bold;text-align:right;">${formatNumber(totalPrice)}</td>
                </tr>
            </tfoot>
        </table>
...
function createAutocompleteDropdown(input, suggestions) {
    ...
    input.onblur = function() {
        setTimeout(() => { if (dropdown) dropdown.remove(); }, 120);
    };
}
...
function enableItemNameAutocomplete(tr) {
    ...
    input.addEventListener('input', function() {
        ...
        const names = getItemNameHistory().filter(n => n.toLowerCase().includes(val));
